...
FROM 
    students st
JOIN 
    attendance a ON st.sid = a.sid 
JOIN 
    sessions se ON a.session_id = se.sessid
WHERE 
    se.cid = $1 
    AND se.did = $2 
    AND se.batch_id = $3
GROUP BY 
    st.s_first_name, st.s_last_name, st.s_roll_number, st.sid
`, [courseId, departmentId, batchId]);
